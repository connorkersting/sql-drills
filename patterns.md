...
| Output shape is part of the spec | the example output shows a column name that does not exist in the source schema, or says "sorted by" / "each ... once" | `SELECT DISTINCT src AS out FROM t WHERE p ORDER BY out` — sorting by the alias works because ORDER BY runs after SELECT | 2026-08-18 | 2 | 2026-08-27 | 2026-08-28 |
